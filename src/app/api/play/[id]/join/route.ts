// src/app/api/play/[id]/join/route.ts
...
export async function POST(req, { params }) {
  ...
  try {
    const participant = await prisma.$transaction(async (tx) => {
      const reqDoc = await tx.playRequest.findUnique({ where: { id }, include: { participants: true }});
      ...
      const already = reqDoc.participants.find((p:any) => p.userId === session.userId);
      if (already) throw { code: "ALREADY_JOINED" };

      const created = await tx.participation.create({ data: { userId: session.userId, playRequestId: id } });
      if (currentCount + 1 >= reqDoc.playersNeeded) await tx.playRequest.update({ where: { id }, data: { status: "FULL" } });
      return created;
    });
